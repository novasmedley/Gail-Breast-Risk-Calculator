...
# Asians have the same conversion factors
for (i in 7:12) {
  F_ <- rbind(F_,c(0.47519806426735,0.50316401683903))
}
F_ <- rbind(F_,c(1,1)) # Presumably Asian Average

# Get race specific values
# Assume race is white for now
raceIdx <- 1
h2 <- h2_[raceIdx,]
h1_star <- h1_star_[raceIdx,]
beta <- beta_[raceIdx,]
F <- F_[raceIdx,]

# Calculate h1, the baseline hazard from a subject without known risk factors
h1 <- rep(1,14) # Initialize
for (i in 1:7)  h1[i] <- h1_star[i]*F[1]
for (i in 8:14) h1[i] <- h1_star[i]*F[2]
